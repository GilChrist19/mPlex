...
# 20220209
#  Update to 20211118
#  The sampling object was updated in the simulation code
#  Updating it here so we have a working example.
#
###############################################################################
...
# Sampling Time
sampDays <- array(data = TRUE, dim = c(simTime, 5, numPatch))

# don't sample eggs, only larvae, pupae, male, female
sampDays[ ,1, ] <- FALSE


# sampling coverage
#  sample larvae 10% per week, converted to daily
#  sample pupae 10% per week, converted to daily
...
for(i in 1:numPC){
  ...
  ####################
  # no output
  combineFiles(mainDir=simDir, workIndicator=25, fPattern=c("L","P","F","M"))
  ...
  readFiles <- c('/000_F.csv','/000_L.csv','/000_P.csv','/000_M.csv')
  writeFiles <- c('/cut_F.csv','/cut_L.csv', '/cut_P.csv', '/cut_M.csv')
