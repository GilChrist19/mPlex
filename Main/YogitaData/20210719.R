...
# 20210719
#  Panmictic test.
#  Yogita's likelihood functions are failing to estimate population size in 
#  a metapopulation. So, we're going to generate a couple sets of data for a 
#  single node - multiple reps, multiple population sizes. 
#  Parameters:
#  Sampling rate - weekly
#  Sampling Fraction - 10% of female pop, 10% of larval pop
#
###############################################################################
...
# male/female get sampledweekly
sampDay[ ,c(2,5)] <- 7

# coverage
#  all aquatic stages get 0% coverage
#  females get 35% coverage
#  males get 23% coverage
sampCov <- matrix(data = c(0,0.10,0,0,0.10), nrow = numPatch, ncol = 5, byrow = TRUE)
...
for(i in 1:numPC){
  ...
  ####################
  # no output
  combineFiles(mainDir=simDir, workIndicator=25, fPattern=c("L","F"))
  ...
  readFiles <- c('/000_F.csv', '/000_L.csv')
  writeFiles <- c('/cut_F.csv', '/cut_L.csv')
  for(cFile in 1:2){
