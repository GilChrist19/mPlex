...
sampCov <- matrix(data = c(0,0,0.10,0.10,0.10)/7, nrow = numPatch, ncol = 5, byrow = TRUE)
...
paramCombo <- as.matrix(expand.grid('nRep' = 1:25,
                                    'nPop' = c(1000) ))
...
for(i in 1:numPC){
  ...
  ####################
  # no output
  combineFiles(mainDir=simDir, workIndicator=25, fPattern=c("P","F","M"))
  ...
  readFiles <- c('/000_F.csv', '/000_P.csv','/000_M.csv')
  writeFiles <- c('/cut_F.csv', '/cut_P.csv', '/cut_M.csv')
