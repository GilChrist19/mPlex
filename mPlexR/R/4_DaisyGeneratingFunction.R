...
DaisyOffspring <- function(fGen, mGen, reference){
  ...
  nDaisies <- length(fSplit)/2

  #make a list of each allele at every locus. This list is length nmPlex, and each
  # sublist has length 2
  fDaisy <- lapply(X = seq.int(from = 1, to = 2*nDaisies, by = 2), FUN = function(X){fSplit[X:(X+1)]})
  mDaisy <- lapply(X = seq.int(from = 1, to = 2*nDaisies, by = 2), FUN = function(X){mSplit[X:(X+1)]})
